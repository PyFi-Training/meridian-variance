...
<style>
    ...
    .brief-box p, .brief-box span, .brief-box div {
        color: #1F2D3D !important;
    }
    .new-plant-box {
        background: #EBF5FB; border: 1px solid #2E86C1;
        border-radius: 6px; padding: 12px 16px;
        margin: 8px 0; font-size: 13px; color: #1F2D3D;
    }
    .timer-box {
        background: #1F2D3D; color: white;
...
for k, v in [("results", None), ("prev_results", None),
             ("chat_history", []), ("last_run_time", None),
             ("last_run_calls", None)]:
...
with st.sidebar:
    ...
    if data_mode == "Pre-loaded demo data":
        ...
        # Preview call count before running
        preview_df = pd.concat(
            [pd.read_csv(p) for p in plant_files_map[month_num]], ignore_index=True
        )
        preview_calls = _count_calls(preview_df)

        st.markdown(f"""<div class='call-count'>
            {len(plant_files_map[month_num])} plants &nbsp;·&nbsp;
            {n_lines} line items &nbsp;·&nbsp;
            <b>{preview_calls} API calls</b>
...
Pipeline handles it automatically. Zero config changes.
    ...
    # Last run stats — only show after a run completes
    if st.session_state.last_run_time is not None:
        st.markdown("---")
        st.markdown("**Last run**")
        st.markdown(f"""<div class='timer-box'>⏱ {st.session_state.last_run_time:.1f}s</div>""",
                    unsafe_allow_html=True)
        if st.session_state.results:
            df_s = st.session_state.results["df"]
            st.metric("Line items processed", len(df_s))
            st.metric("Plants", df_s["plant"].nunique())
            st.metric("API calls fired", st.session_state.last_run_calls)
            st.metric("Dept groups", df_s.groupby(["plant","department"]).ngroups)
            st.caption("**What fired:**")
            st.caption(f"• {len(df_s)} line commentary calls")
            st.caption(f"• {len(df_s)} severity classifier calls")
            st.caption(f"• {df_s.groupby(['plant','department']).ngroups} dept summary calls")
            st.caption(f"• {df_s['plant'].nunique()} plant brief calls")
            st.caption("• 1 consolidated CFO brief call")
            st.caption("All concurrent via asyncio.gather()")
...
if run_btn:
    ...
    label = demo_choice if demo_choice else \
            f"Upload: {', '.join(f.name for f in uploaded_files)}"

    with st.spinner("Loading data and computing variances..."):
        df = load_data()
    ...
    status_placeholder.info(
        f"🔄 Firing **{total_calls} concurrent API calls** across 3 passes:\n\n"
        f"- Pass 1: {len(df)} line-item commentary calls\n"
        f"- Pass 2: {len(df)} severity + "
        f"{df.groupby(['plant','department']).ngroups} dept summary calls\n"
        f"- Pass 3: {df['plant'].nunique()} plant briefs + 1 CFO brief"
    )

    # Run timer in thread while pipeline executes
    import threading
    running = [True]
    ...
    def update_timer():
        while running[0]:
            ...
            timer_placeholder.markdown(
                f'<div class="timer-box">⏱ {elapsed:.1f}s</div>',
                unsafe_allow_html=True,
            )
            time.sleep(0.1)
    ...
    timer_placeholder.markdown(
        f'<div class="timer-box">✓ Done in {elapsed:.1f}s — {total_calls} calls</div>',
        unsafe_allow_html=True,
    )
    ...
    st.session_state.last_run_calls = total_calls
    st.rerun()
...
if not st.session_state.results:
    ...
    with st.expander("📋 Expected CSV format"):
        st.code(
            ...
            "Plant A,Cost of Sales,Raw Materials,9200000,10380000,False",
            language="csv",
        )
    st.stop()
...
with t1:
    ...
    k5.metric("Plants Analysed", df["plant"].nunique())
    ...
    for i, plant in enumerate(plants):
        ...
        with cols[i]:
            ...
            st.metric("Gross Profit", fmt_dollars(pgp_act),
                      fmt_dollars(pgp_act - pgp_bud))

    st.divider()
    st.subheader("Consolidated CFO Brief")

    # Render as plain text to avoid CSS colour conflicts
    for para in r["cfo_brief"].split("\n"):
        if para.strip():
            st.markdown(para)
    ...
    for plant in fd["plant"].unique():
        ...
        for dept in fd[fd["plant"] == plant]["department"].unique():
            ...
            st.dataframe(disp, use_container_width=True, hide_index=True,
                         column_config={
                             "AI Commentary": st.column_config.TextColumn(width="large"),
...
with t3:
    if prev is None:
        ...
        st.markdown("3. Come back here — see which lines worsened and which improved")
        st.markdown("4. Select **Month 3** → Run — Plant C appears automatically")
    else:
        curr_df = r["df"]
        prev_df = prev["df"]
        st.markdown(f"**{prev['label'][:40]}**  →  **{r['label'][:40]}**")
        st.divider()

        def totals(d):
            rv = d[d["is_revenue"]]; co = d[~d["is_revenue"]]
            return {
                "rev":  rv["actual"].sum(), "rev_bud": rv["budget"].sum(),
        ...
        m2.metric("Total Costs",  fmt_dollars(c["cost"]),
...
with t4:
    st.caption(
        "Full context — all line items, department summaries, plant briefs, and CFO brief — "
        "injected automatically. No pasting required."
    )
    ...
    if prompt := st.chat_input("e.g. Compare raw material trends across all plants"):
        ...
        messages = [{
            ...
            "content": (
                ...
                f"Unfavourable variances in parentheses. Be concise.\n\n"
                f"VARIANCE DATA:\n{context}"
            ),
